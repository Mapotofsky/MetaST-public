...
from models.metast.pretrained_encoders.softclt_ts2vec.utils import pkl_load, pad_nan_to_target, split_with_nan
from models.metast.pretrained_encoders.softclt_ts2vec.utils_distance_matrix import *
...
def load_UCR(dataset, type_="DTW"):
    ...
    if dataset not in [
        ...
        "MelbournePedestrian",
        "PickupGestureWiimoteZ", "PigAirwayPressure", "PigArtPressure", "PigCVP", "PLAID", "PowerCons",
        "Rock",
        "SemgHandGenderCh2", "SemgHandMovementCh2", "SemgHandSubjectCh2", "ShakeGestureWiimoteZ", "SmoothSubspace",
        "UMD",
    ]:
        return (
            train[..., np.newaxis],
        ...
        if max_train_length is not None:
            ...
            if sections >= 2:
                train_X = np.concatenate(
                    split_with_nan(train_X, sections, axis=1), axis=0
                )
    ...
    if train.ndim == 2:
        if dataset not in [
            ...
            return (
                train[..., np.newaxis],
                train_labels,
                train_perc1[..., np.newaxis],
                train_perc1_labels,
                train_perc5[..., np.newaxis],
                train_perc5_labels,
                test[..., np.newaxis],
                test_labels,
                sim_mat,
        ...
        train = scaler.transform(train.reshape(-1, train.shape[-1])).reshape(
            train.shape
        )
        train_perc1 = scaler.transform(
            train_perc1.reshape(-1, train_perc1.shape[-1])
        ).reshape(train_perc1.shape)
        train_perc5 = scaler.transform(
            train_perc5.reshape(-1, train_perc5.shape[-1])
        ).reshape(train_perc5.shape)
...
def load_forecast_npy(name, max_train_length, univar=False, type_="DTW"):
    ...
    if os.path.exists(DIST_MAT_PATH):
        ...
        # 如果距离矩阵文件不存在，则通过预定义的距离度量计算各个特征维度之间的距离
        print(f"Saving & Loading {type_} ...")
        if max_train_length is not None:
