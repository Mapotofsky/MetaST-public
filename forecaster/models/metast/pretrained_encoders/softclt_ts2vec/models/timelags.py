...
def dup_matrix(mat):  # (N, N) -> (N, 2N - 1)
    mat0 = torch.tril(mat, diagonal=-1)[:, :-1]  # (N, N - 1)
...
def timelag_sigmoid_window(T, sigma=1, window_ratio=1.0):
    ...
    matrix = 2 / (1 + np.exp(dist * sigma))
    matrix = np.where(matrix < 1e-6, 0, matrix)
    dist_from_diag = np.abs(
        np.subtract.outer(np.arange(dist.shape[0]), np.arange(dist.shape[1]))
    )
