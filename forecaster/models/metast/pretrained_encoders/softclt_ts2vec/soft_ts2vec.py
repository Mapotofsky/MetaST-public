...
from models.metast.pretrained_encoders.softclt_ts2vec.models import TSEncoder
from models.metast.pretrained_encoders.softclt_ts2vec.models.hard_losses import *
from models.metast.pretrained_encoders.softclt_ts2vec.models.soft_losses import *
from models.metast.pretrained_encoders.softclt_ts2vec.utils import *
from models.metast.pretrained_encoders.softclt_ts2vec.utils_distance_matrix import save_sim_mat
from models.metast.pretrained_encoders.softclt_ts2vec.datautils import normalize_TS
...
class TS2Vec:
    ...
    def fit(
        ...
        soft_labels, run_dir, n_epochs=None, n_iters=None,
        ...
        Args:
            ...
            n_iters (Union[int, NoneType]): The number of iterations. When this reaches, the training stops.
                                            If both n_epochs and n_iters are not specified, a default setting would be used that
                                            sets n_iters to 200 for a dataset with size <= 100000, 600 otherwise.
        ...
        #   (n_instance, n_timestamps, n_features) -> (n_instance, n_timestamps)
        #   (n_instance, n_timestamps) -> (n_timestamps)
        ...
        while True:
            ...
            interrupted = False
            for x, idx in train_loader:  # x.shape = (batch_size, ts_l, n_features)

                if n_iters is not None and self.n_iters >= n_iters:
                    interrupted = True
                    break
                if soft_labels is None:
                    data_for_sim = x[..., 0].numpy()
                    soft_labels_batch = save_sim_mat(normalize_TS(data_for_sim), min_=0, max_=1,
                                                     multivariate=False, type_="COS")
                else:
                    # soft_labels.shape = (N, N - 1)
                    # soft_labels_batch.shape = (batch_size, batch_size)
    ...
    def encode(
        ...
        Args:
            data (numpy.ndarray): This should have a shape of (n_instance, n_timestamps, n_features).
                                  All missing data should be set to NaN.
            ...
            encoding_window (Union[str, int]): When this param is specified, the computed representation would the max pooling over this window.
                                               This can be set to 'full_series', 'multiscale' or an integer specifying the pooling kernel size.
            casual (bool): When this param is set to True, the future informations would not be encoded into representation of each timestamp.
            sliding_length (Union[int, NoneType]): The length of sliding window.
        ...
        with torch.no_grad():
            ...
            output = torch.cat(output, dim=0)  # (n_samples, C)
    ...
    def encode_tensor(
        ...
        """Tensorized encode method for use as a model component.

        Args:
            data (torch.Tensor): Input tensor with shape (n_instance, n_timestamps, n_features).
            mask (str): The mask used by encoder can be specified with this parameter.
                        This can be set to 'binomial', 'continuous', 'all_true', 'all_false' or 'mask_last'.
            encoding_window (Union[str, int]): When this param is specified, the computed
                        representation would the max pooling over this window. This can be set to
                        'full_series', 'multiscale' or an integer specifying the pooling kernel size.
            casual (bool): When this param is set to True, the future informations would
                        not be encoded into representation of each timestamp.
            sliding_length (Union[int, NoneType]): The length of sliding window. When this param
                        is specified, a sliding inference would be applied on the time series.
            sliding_padding (int): This param specifies the contextual data length used
                        for inference every sliding windows.

        Returns:
            torch.Tensor: The representations for data.
        """
        assert self.net is not None, "please train or load a net first"
        assert data.ndim == 3, "Input data should have 3 dimensions (n_instance, n_timestamps, n_features)"
        ...
        with torch.no_grad():
            if sliding_length is not None:
                num_windows = (ts_l + sliding_length - 1) // sliding_length
                reprs = []

                for i in range(num_windows):
                    start_idx = i * sliding_length
                    l = start_idx - sliding_padding
                    r = start_idx + sliding_length + (sliding_padding if not casual else 0)

                    actual_l = max(l, 0)
                    actual_r = min(r, ts_l)

                    x_sliding = data[:, actual_l:actual_r]

                    if l < 0:
                        left_pad = torch.full(
                            (n_samples, -l, data.size(-1)), float('nan'),
                            device=data.device, dtype=data.dtype)
                        x_sliding = torch.cat([left_pad, x_sliding], dim=1)

                    if r > ts_l:
                        right_pad = torch.full(
                            (n_samples, r - ts_l, data.size(-1)), float('nan'),
                            device=data.device, dtype=data.dtype)
                        x_sliding = torch.cat([x_sliding, right_pad], dim=1)
                ...
                if encoding_window == "full_series":
                    ...
                    ).squeeze(-1)  # (n_samples, output_dims)
            else:
                output = self._eval_with_pooling(
                    data, mask, encoding_window=encoding_window
                )
                if encoding_window == "full_series":
                    output = output.squeeze(1)  # (n_samples, output_dims)
